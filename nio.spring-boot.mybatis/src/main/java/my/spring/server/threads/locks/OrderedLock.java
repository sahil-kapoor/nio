...
import my.spring.server.threads.things.Queue;
...
public class OrderedLock implements ILock, ISchedulingRule {
	...
	if (DEBUG)
	    System.out.println("[" //$NON-NLS-1$
		    + Thread.currentThread() + "] Operation waiting to be executed... " //$NON-NLS-1$
		    + this);
	...
	if (DEBUG)
	    System.out.println("[" //$NON-NLS-1$
		    ...
		    + (success ? "] Operation started... " //$NON-NLS-1$
	...
	try {
	    ...
	    if (DEBUG)
		System.out.println("[" //$NON-NLS-1$
			+ currentThread + "] Operation interrupted while waiting... :-|"); //$NON-NLS-1$
