...
import my.spring.server.exceptions.CoreException;
...
public class LockManager {
	...
	if (e instanceof CoreException) {
	    ...
	    status = new MultiStatus(JobManager.PI_JOBS, JobManager.PLUGIN_ERROR, "LockManager.handleException", //$NON-NLS-1$
		    e);
	    ((MultiStatus) status).merge(((CoreException) e).getStatus());
	} else {
	    status = new Status(IStatus.ERROR, JobManager.PI_JOBS, JobManager.PLUGIN_ERROR,
		    "LockManager.handleException", //$NON-NLS-1$
		    e);
	}
    }
