...
public final class QualifiedName {
    ...
    /**
     * Creates and returns a new qualified name with the given qualifier and
     * local name. The local name must not be the empty string. The qualifier
     ...
     * 
     * @param qualifier
     *            the qualifier string, or <code>null</code>
     * @param localName
     ...
     * <p>
     * Qualified names are equal if and only if they have the same qualified
     * parts and local parts. Qualified names are not equal to objects other
    ...
    /**
     * Converts this qualified name into a string, suitable for debug purposes
     * only.
     */
    @Override
    public String toString() {
	return (getQualifier() == null ? "" //$NON-NLS-1$
		: getQualifier() + ':') + getLocalName();
    }
}
