...
import my.spring.server.threads.status.MultiStatus;
...
public class DeadlockDetector {
	...
	if (threadIndex < 0) {
		...
		System.out
			.println("[lockReleased] Lock " + lock + " was already released by thread " + owner.getName());
	    return;
	}
	if (lockIndex < 0) {
	    if (JobManager.DEBUG_LOCKS)
	...
	for (int j = 0; j < graph[threadIndex].length; j++) {
	    if ((lock.isConflicting((ISchedulingRule) locks.get(j))) || (!(lock instanceof ILock)
		    && !(locks.get(j) instanceof ILock) && (graph[threadIndex][j] > NO_STATE))) {
		if (graph[threadIndex][j] == NO_STATE) {
		    if (JobManager.DEBUG_LOCKS)
			System.out.println("[lockReleased] More releases than acquires for thread " + owner.getName()
				+ " and lock " + lock);
		} else {
		    graph[threadIndex][j]--;
		}
	...
	if (threadIndex < 0) {
	    if (JobManager.DEBUG_LOCKS)
		System.out.println(
			"[lockReleasedCompletely] Lock " + rule + " was already released by thread " + owner.getName());
	    return;
	}
	if (ruleIndex < 0) {
	    if (JobManager.DEBUG_LOCKS)
		System.out.println(
			"[lockReleasedCompletely] Thread " + owner.getName() + " already released lock " + rule);
	    return;
	}
	/**
	...
	if (graph[threadIndex][lockIndex] != WAITING_FOR_LOCK)
	    Assert.isTrue(false, "Thread " + owner.getName() + " was not waiting for lock " + lock.toString()
		    + " so it could not time out.");
	graph[threadIndex][lockIndex] = NO_STATE;
	reduceGraph(threadIndex, lock);
    }
	...
	graph = tempGraph;
	Assert.isTrue(numThreads == graph.length, "Rows and threads don't match.");
	Assert.isTrue(numLocks == ((graph.length > 0) ? graph[0].length : 0), "Columns and locks don't match.");
    }
	...
	String msg = "Deadlock detected. All locks owned by thread " + deadlock.getCandidate().getName()
		+ " will be suspended.";
	MultiStatus main = new MultiStatus(JobManager.PI_JOBS, JobManager.PLUGIN_ERROR, msg,
	...
	out.println(" :: ");
	for (int j = 0; j < locks.size(); j++) {
	    out.print(" " + locks.get(j) + ',');
	}
	out.println();
	for (int i = 0; i < graph.length; i++) {
	    out.print(" " + ((Thread) lockThreads.get(i)).getName() + " : ");
	    for (int j = 0; j < graph[i].length; j++) {
		out.print(" " + graph[i][j] + ',');
	    }
	    out.println();
