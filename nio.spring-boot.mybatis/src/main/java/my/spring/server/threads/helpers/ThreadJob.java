...
public class ThreadJob extends Job {
	...
	if (top >= 0 && top < ruleStack.length) {
		...
		buf.append(", but the rule stack was out of bounds: " + top);
	}
	...
	try {
	    ...
	    IStatus status = new Status(IStatus.ERROR, JobManager.PI_JOBS, JobManager.PLUGIN_ERROR,
		    "ThreadJob.isCanceled", e);
	}
	return false;
    }
    ...
    static ThreadJob waitForRun(ThreadJob threadJob, IProgressMonitor monitor, InternalJob blockingJob,
	    Thread blocker) {
