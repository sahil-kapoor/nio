...
public class ImplicitJobs {
	 ...
	 */
	private final Set<ISchedulingRule> suspendedRules = new HashSet<ISchedulingRule>(
			20);

	/**
	 * Maps (Thread->ThreadJob), threads to the currently running job for that
	 * thread. @GuardedBy("this")
	 */
	private final Map<Thread, ThreadJob> threadJobs = new HashMap<Thread, ThreadJob>(
			20);
	...
	void begin(ISchedulingRule rule, IProgressMonitor monitor,
		...
		synchronized (this) {
			...
			if (rule == null)
				return;
			// create a thread job for this thread, use the rule from the real
			// job if it has one
			...
			// indicate if it is a system job to ensure isBlocking works
			// correctly
		...
		try {
			...
			if (threadJob.acquireRule) {
				// no need to re-acquire any locks because the thread did not
				// wait to get this lock
				if (manager.runNow(threadJob, false) == null)
					manager.getLockManager()
							.addLockThread(Thread.currentThread(), rule);
				else
					threadJob = ThreadJob.joinRun(threadJob, monitor);
			}
		} finally {
			// remember this thread job - only do this
			// after the rule is acquired because it is ok for this thread to
			// acquire
	...
	synchronized void end(ISchedulingRule rule, boolean resume) {
		...
		if (threadJob == null)
			Assert.isLegal(rule == null,
					"endRule without matching beginRule: " + rule);
	...
	/**
	 * Called when a worker thread has finished running a job. At this point,
	 * the worker thread must not own any scheduling rules
	 * 
	 * @param lastJob
	 *            The last job to run in this thread
	 */
	public void endJob(InternalJob lastJob) {
		final Thread currentThread = Thread.currentThread();
		IStatus error;
		synchronized (this) {
			...
			String msg = "Worker thread ended job: " + lastJob
					+ ", but still holds rule: " + threadJob;
	...
	/**
	 * Returns true if this rule has been suspended, and false
	 * otherwise. @GuardedBy("this")
	 */
	private boolean isSuspended(ISchedulingRule rule) {
		if (suspendedRules.size() == 0)
			return false;
		for (Iterator<ISchedulingRule> it = suspendedRules.iterator(); it
				.hasNext();)
	...
	/**
	 * Indicates that a thread job is no longer in use and can be
	 * reused. @GuardedBy("this")
	 */
	...
	synchronized void transfer(ISchedulingRule rule, Thread destinationThread) {
		...
		Assert.isLegal(target == null,
				"Transfer rule to job that already owns a rule");
		// ensure calling thread owns the job being transferred
		ThreadJob source = threadJobs.get(currentThread);
		Assert.isNotNull(source, "transferRule without beginRule");
		Assert.isLegal(source.getRule() == rule, "transferred rule " + rule
				+ " does not match beginRule: " + source.getRule()); // transfer
																		// the
																		// thread
																		// job
																		// without
	...
	synchronized void addWaiting(ThreadJob threadJob) {
		synchronized (((InternalJob) threadJob).jobStateLock) {
			...
			notifyWaitingThreadJobs(threadJob);
			((InternalJob) threadJob)
					.setWaitQueueStamp(manager.waitQueueCounter.increment());
		}
