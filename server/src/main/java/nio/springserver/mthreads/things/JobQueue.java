...
public final class JobQueue {
	...
	/**
	 * If true, conflicting jobs will be allowed to overtake others in the queue
	 * that have lower priority. If false, higher priority jumps can only move
	 * up the queue by overtaking jobs that they don't conflict with.
	 */
	...
	public void enqueue(InternalJob newEntry) {
		// assert new entry is does not already belong to some other data
		// structure
	...
	private boolean canOvertake(InternalJob newEntry, InternalJob queueEntry) {
		...
		if (queueEntry == dummy)
			return false;
		// if the new entry was already in the wait queue, ensure it is
		// re-inserted in correct position (bug 211799)
		if (newEntry.getWaitQueueStamp() > 0 && newEntry
				.getWaitQueueStamp() < queueEntry.getWaitQueueStamp())
			return true;
		// if the new entry has lower priority, there is no need to overtake the
		// existing entry
		if (allowPriorityOvertaking && queueEntry.compareTo(newEntry) >= 0)
			return false;
		// the new entry has higher priority, but only overtake the existing
		// entry if the queue allows it
