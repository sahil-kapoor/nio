...
/**
 * A MultiRule is a compound scheduling rule that represents a fixed group of
 * child scheduling rules. A MultiRule conflicts with another rule if any of its
 * children conflict with that rule. More formally, a compound rule represents a
 * logical intersection of its child rules with respect to the
 * <code>isConflicting</code> equivalence relation.
 * <p>
 * A MultiRule will never contain other MultiRules as children. If a MultiRule
 * is provided as a child, its children will be added instead.
 * </p>
 * 
 * @since 3.0
...
public class MultiRule implements ISchedulingRule {
	...
	/**
	 * Returns a scheduling rule that encompasses all provided rules. The
	 * resulting rule may or may not be an instance of <code>MultiRule</code>.
	 * If all provided rules are <code>null</code> then the result will be
	 * <code>null</code>.
	 * 
	 * @param ruleArray
	 *            An array of scheduling rules, some of which may be
	 *            <code>null</code>
	...
	/**
	 * Returns a scheduling rule that encompasses both provided rules. The
	 * resulting rule may or may not be an instance of <code>MultiRule</code>.
	 * If both provided rules are <code>null</code> then the result will be
	 * <code>null</code>.
	 * 
	 * @param rule1
	 *            a scheduling rule, or <code>null</code>
	 * @param rule2
	 *            another scheduling rule, or <code>null</code>
	 * @return a combined scheduling rule, or <code>null</code>
	 */
	...
	/*
	 * Collapses an array of rules that may contain MultiRules into an array in
	...
	private static ISchedulingRule[] flatten(ISchedulingRule[] nestedRules) {
		...
		for (int i = 0; i < nestedRules.length; i++) {
			if (nestedRules[i] instanceof MultiRule) {
				ISchedulingRule[] children = ((MultiRule) nestedRules[i])
						.getChildren();
		...
		return (ISchedulingRule[]) myRules
				.toArray(new ISchedulingRule[myRules.size()]);
	}
	...
	/**
	 * Creates a new scheduling rule with no nested rules. For internal use
	 * only.
	 */
	private MultiRule() {
	...
	public boolean contains(ISchedulingRule rule) {
		...
		if (rule instanceof MultiRule) {
			...
			// for each child of the target, there must be some child in this
