...
import nio.springserver.mthreads.helpers.IJobManager;
...
import nio.springserver.mthreads.things.QualifiedName;
 ...
 * reusable).
 * <p>
 * Jobs have a state that indicates what they are currently doing. When
 * constructed, jobs start with a state value of <code>NONE</code>. When a job
 * is scheduled to be run, it moves into the <code>WAITING</code> state. When a
 * job starts running, it moves into the <code>RUNNING</code> state. When
 * execution finishes (either normally or through cancelation), the state
 * changes back to <code>NONE</code>.
 * </p>
 * <p>
 * A job can also be in the <code>SLEEPING</code> state. This happens if a user
 * calls Job.sleep() on a waiting job, or if a job is scheduled to run after a
 * specified delay. Only jobs in the <code>WAITING</code> state can be put to
 * sleep. Sleeping jobs can be woken at any time using Job.wakeUp(), which will
 * put the job back into the <code>WAITING</code> state.
 * </p>
 * <p>
 * Jobs can be assigned a priority that is used as a hint about how the job
 * should be scheduled. There is no guarantee that jobs of one priority will be
 * run before all jobs of lower priority. The javadoc for the various priority
 * constants provide more detail about what each priority means. By default,
 * jobs start in the <code>LONG</code> priority class.
 * 
 * @see IJobManager
 * @since 3.0
 */
public abstract class Job extends InternalJob {
	...
	public static final IStatus ASYNC_FINISH = new Status(IStatus.OK,
			JobManager.PI_JOBS, 1, "OK", null);

	/* Job priorities */
	/**
	 * Job priority constant (value 10) for interactive jobs. Interactive jobs
	 * generally have priority over all other jobs. Interactive jobs should be
	 * either fast running or very low on CPU usage to avoid blocking other
	 * interactive jobs from running.
	 * 
	 * @see #getPriority()
	 * @see #setPriority(int)
	...
	/**
	 * Job priority constant (value 40) for build jobs. Build jobs are generally
	 * run after all other background jobs complete.
	 * 
	 * @see #getPriority()
	 * @see #setPriority(int)
	 * @see #run(IProgressMonitor)
	...
	/**
	 * Job priority constant (value 50) for decoration jobs. Decoration jobs
	 * have lowest priority. Decoration jobs generally compute extra information
	 * that the user may be interested in seeing but is generally not waiting
	 * for.
	 * 
	 * @see #getPriority()
	 * @see #setPriority(int)
	...
	/**
	 * Registers a job listener with this job Has no effect if an identical
	 ...
	 * returns <code>false</code>. Overriding implementations must return
	 * <code>false</code> for families they do not recognize.
	 * </p>
	 * 
	 * @param family
	 ...
	 * are finished by calling this method. This method must not be called by a
	 * job that has not indicated that it is executing asynchronously.
	 * <p>
	 * This method must not be called from within the scope of a job's
	 * <code>run</code> method. Jobs should normally indicate completion by
	 * returning an appropriate status from the <code>run</code> method. Jobs
	 * that return a status of <code>ASYNC_FINISH</code> from their run method
	 * must later call <code>done</code> to indicate completion.
	 * 
	 * @param result
	 *            a status object indicating the result of the job's execution.
	 * @see #ASYNC_FINISH
	 * @see #run(IProgressMonitor)
	 */
	@Override
	...
	/**
	 * Returns the priority of this job. The priority is used as a hint when the
	 * job is scheduled to be run.
	 * 
	...
	/**
	 * Returns the value of the property of this job identified by the given
	 * key, or <code>null</code> if this job has no such property.
	 *
	 * @param key
	...
	/**
	 * Returns the scheduling rule for this job. Returns <code>null</code> if
	 * this job has no scheduling rule.
	 * 
	...
	/**
	 * Returns whether this job is blocking a higher priority non-system job
	 * from starting due to a conflicting scheduling rule. Returns
	 * <code>false</code> if this job is not running, or is not blocking a
	 * higher priority non-system job.
	 * 
	 * @return <code>true</code> if this job is blocking a higher priority
	 *         non-system job, and <code>false</code> otherwise.
	 * @see #getRule()
	 * @see #isSystem()
	 */
	@Override
	 ...
	 * revealed to users in any UI presentation of jobs. Other than their UI
	 * presentation, system jobs act exactly like other jobs. If this value is
	 * not explicitly set, jobs are treated as non-system jobs. The default
	 ...
	 * These jobs may be presented differently in the UI. The default value is
	 * <code>false</code>.
	 * 
	...
	/**
	 * Removes a job listener from this job. Has no effect if an identical
	...
	/**
	 * Associates this job with a progress group. Progress feedback on this
	 * job's next execution will be displayed together with other jobs in that
	 * group. The provided monitor must be a monitor created by the method
	 * <tt>IJobManager.createProgressGroup</tt> and must have at least
	 * <code>ticks</code> units of available work.
	 * <p>
	 * The progress group must be set before the job is scheduled. The group
	 * will be used only for a single invocation of the job's <tt>run</tt>
	 * method, after which any association of this job to the group will be
	 * lost.
	 * 
	 * @see IJobManager#createProgressGroup()
	 * @param group
	 *            The progress group to use for this job
	 * @param ticks
	 *            the number of work ticks allocated from the parent monitor, or
	 ...
	 * If the supplied value is <code>null</code>, the property is removed from
	 * this resource.
	 * <p>
	 * Properties are intended to be used as a caching mechanism by ISV
	 * plug-ins. They allow key-object associations to be stored with a job
	 * instance. These key-value associations are maintained in memory (at all
	 * times), and the information is never discarded automatically.
	 * </p>
	 * <p>
	 * The qualifier part of the property name must be the unique identifier of
	 * the declaring plug-in (e.g. <code>"com.example.plugin"</code>).
	 * </p>
	 *
	 * @param key
	 ...
	 * 
	 * @param rule
	 *            the new scheduling rule, or <code>null</code> if the job
	...
	/**
	 * Sets whether or not this job is a system job. System jobs are typically
	 * not revealed to users in any UI presentation of jobs. Other than their UI
	 * presentation, system jobs act exactly like other jobs. If this value is
	 * not explicitly set, jobs are treated as non-system jobs. This method must
	 * be called before the job is scheduled.
	 * 
	 * @param value
	 *            <code>true</code> if this job should be a system job, and
	 *            <code>false</code> otherwise.
	 * @see #isSystem()
	 */
	@Override
	...
	/**
	 * Sets whether or not this job has been directly initiated by a UI end
	 * user. These jobs may be presented differently in the UI. This method must
	 * be called before the job is scheduled.
	 * 
	 * @param value
	...
	/**
	 * Requests that this job be suspended. If the job is currently waiting to
	 * be run, it will be removed from the queue move into the {@link #SLEEPING}
	 * state. The job will remain asleep until either resumed or canceled. If
	 * this job is not currently waiting to be run, this method has no effect.
	 * <p>
	 * Sleeping jobs can be resumed using <code>wakeUp</code>.
	 * 
	 * @return <code>false</code> if the job is currently running (and thus
	 *         cannot be put to sleep), and <code>true</code> in all other cases
	 * @see #wakeUp()
	 */
	@Override
	...
	/**
	 * Returns a string representation of this job to be used for debugging
	 * purposes only.
	 * 
	 ...
	 * <p>
	 * This is a convenience method, fully equivalent to <code>wakeUp(0L)</code>
	 * .
	 * </p>
	 * 
	 * @see #sleep()
	...
	/**
	 * Puts this job back into the {@link #WAITING} state after the specified
	 * delay. This is equivalent to canceling the sleeping job and rescheduling
	 * with the given delay. If this job is not currently sleeping, the request
	 * is ignored.
	 * 
	 * @param delay
	...
	/**
	 * Temporarily puts this <code>Job</code> back into {@link #WAITING} state
	 * and relinquishes the job's scheduling rule so that any {@link #WAITING}
	 * jobs that conflict with this job's scheduling rule have an opportunity to
	 * start. This method will wait until the rule this job held prior to
	 * invoking this method is re-acquired. This method has no effect and
	 * returns <tt>null</tt> if there are no {@link #WAITING} jobs that conflict
	 * with this job's scheduling rule.
	 * <p>
	 * Note: If this job has acquired any other locks, implicit or explicit,
	 * they will <i>not</i> be released. This may increase the risk of deadlock,
	 * so this method should only be used when it is known that the environment
	 * is safe.
	 * <p>
	 * This method must be invoked by this job's <code>Thread</code>, and only
	 * when it is {@link #RUNNING} state.
	 * <p>
	 * 
	 * @param monitor
	 *            a progress monitor, or <tt>null</tt> if progress reporting is
	 *            not desired. Cancellation attempts will be ignored.
	 * @return The Job that was {@link #WAITING}, and blocked by this
	 *         <code>Job</code> (at the time this method was invoked) that was
	 *         unblocked and allowed a chance to run, or <tt>null</tt> if no
