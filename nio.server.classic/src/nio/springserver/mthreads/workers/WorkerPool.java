package nio.springserver.mthreads.workers;
...
import nio.springserver.mthreads.helpers.ThreadJob;
...
public class WorkerPool {
	...
	public WorkerPool(JobManager manager) {
		this.manager = manager;
		this.defaultContextLoader = Thread.currentThread()
	 ...
	 * OutOfMemoryError conditions and thus must be paranoid about allocating
	 * objects.
	 */
	protected void endJob(InternalJob job, IStatus result) {
		...
		if ((job.getRule() != null) && !(job instanceof ThreadJob)) {
			...
			manager.getLockManager().removeLockCompletely(
					Thread.currentThread(), job.getRule());
		}
	...
	/**
	 * Signals the death of a worker thread. Note that this method can be called
	 * under OutOfMemoryError conditions and thus must be paranoid about
	 * allocating objects.
	 */
	protected synchronized void endWorker(Worker worker) {
		if (remove(worker) && JobManager.DEBUG)
			JobManager.debug("worker removed from pool: " //$NON-NLS-1$
					+ worker);
	}
	...
	private synchronized void incrementBusyThreads() {
		...
		if (++busyThreads > numThreads) {
			if (JobManager.DEBUG)
				Assert.isTrue(false,
						Integer.toString(busyThreads) + ',' + numThreads);
	...
	private synchronized boolean remove(Worker worker) {
		for (int i = 0; i < threads.length; i++) {
			if (threads[i] == worker) {
				System.arraycopy(threads, i + 1, threads, i,
						numThreads - i - 1);
	...
	private synchronized void sleep(long duration) {
		...
		busyThreads--;
		if (JobManager.DEBUG)
			JobManager.debug("worker sleeping for: " //$NON-NLS-1$
	...
	protected InternalJob startJob(Worker worker) {
		...
		synchronized (this) {
			if (!manager.isActive()) {
				// must remove the worker immediately to prevent all threads
				// from expiring
				endWorker(worker);
				return null;
			}
		...
		try {
			...
			while (manager.isActive() && job == null) {
				...
				job = manager.startJob();
				// if we were already idle, and there are still no new jobs,
				// then
				// the thread can expire
				synchronized (this) {
					if (job == null
							&& (System.currentTimeMillis()
									- idleStart > BEST_BEFORE)
							&& (numThreads - busyThreads) > MIN_THREADS) {
						// must remove the worker immediately to prevent all
						// threads from expiring
						endWorker(worker);
						return null;
					}
				}
				// if we didn't sleep but there was no job available, make sure
				// we sleep to avoid a tight loop (bug 260724)
				if (hint <= 0 && job == null)
					sleep(50);
			}
			if (job != null) {
				// if this job has a rule, then we are essentially acquiring a
				// lock
				if ((job.getRule() != null) && !(job instanceof ThreadJob)) {
					// don't need to re-acquire locks because it was not
					// recorded in the graph
					// that this thread waited to get this rule
					manager.getLockManager().addLockThread(
							Thread.currentThread(), job.getRule());
				}
