 ...
 * and the threads that own them. All the relationships are stored in a 2D
 * integer array. The rows in the array are threads, while the columns are
 * locks. Two corresponding arrayLists store the actual threads and locks. The
 * index of a thread in the first arrayList is the index of the row in the
 * graph. The index of a lock in the second arrayList is the index of the column
 * in the graph. An entry greater than 0 in the graph is the number of times a
 * thread in the entry's row acquired the lock in the entry's column. An entry
 * of -1 means that the thread is waiting to acquire the lock. An entry of 0
 * means that the thread and the lock have no relationship.
 * 
 * The difference between rules and locks is that locks can be suspended, while
 * rules are implicit locks and as such cannot be suspended. To resolve
 * deadlock, the graph will first try to find a thread that only owns locks.
 * Failing that, it will find a thread in the deadlock that owns at least one
 * lock and suspend it.
 * 
 * Deadlock can only occur among locks, or among locks in combination with
 * rules. Deadlock among rules only is impossible. Therefore, in any deadlock
 * one can always find a thread that owns at least one lock that can be
 * suspended.
 * 
 ...
 * 
 * The implementation of the graph will sometimes also find and resolve bogus
 * deadlocks. graph: assuming this rule hierarchy: R2 R3 L1 R1 J1 1 0 0 / \ J2 0
 * 1 -1 R2 R3 J3 -1 0 1
 * 
 * If in the above situation job4 decides to acquire rule1, then the graph will
 * transform to the following: R2 R3 R1 L1 J1 1 0 1 0 J2 1 1 1 -1 J3 -1 0 0 1 J4
 * 0 0 -1 0
 * 
 * and the graph will assume that job2 and job3 are deadlocked and suspend lock1
 * of job3. The reason the deadlock is bogus is that the deadlock is unlikely to
 * actually happen (the threads are currently not deadlocked, but might deadlock
 * later on when it is too late to detect it) Therefore, in order to make sure
 * that no deadlock is possible, the deadlock will still be resolved at this
 * point.
 */
public class DeadlockDetector {
	...
	/**
	 * Recursively check if any of the threads that prevent the current thread
	 * from running are actually deadlocked with the current thread. Add the
	 * threads that form deadlock to the deadlockedThreads list.
	 */
	private boolean addCycleThreads(ArrayList deadlockedThreads, Thread next) {
		...
		// if the thread is not blocked by other threads, then it is not part of
		// a deadlock
		if (blocking.length == 0)
			return false;
		boolean inCycle = false;
		for (int i = 0; i < blocking.length; i++) {
			// if we have already visited the given thread, then we found a
			// cycle
	 ...
	 * deadlock is detected return true, else return false.
	 */
	private boolean checkWaitCycles(int[] waitingThreads, int lockIndex) {
		/**
		 * find the lock that this thread is waiting for recursively check if
		 * this is a cycle (i.e. a thread waiting on itself)
		 */
		for (int i = 0; i < graph.length; i++) {
			if (graph[i][lockIndex] > NO_STATE) {
				...
				// this thread is not involved in a cycle yet, so remove the
				// visited flag
				waitingThreads[i]--;
			}
		}
	...
	/**
	 * Returns true IFF the matrix contains a row for the given thread. (meaning
	 * the given thread either owns locks or is waiting for locks)
	 */
	...
	/**
	 * A new rule was just added to the graph. Find a rule it conflicts with and
	 * update the new rule with the number of times it was acquired implicitly
	 * when threads acquired conflicting rule.
	 */
	private void fillPresentEntries(ISchedulingRule newLock, int lockIndex) {
		...
		for (int j = 0; j < locks.size(); j++) {
			if ((j != lockIndex) && (newLock
					.isConflicting((ISchedulingRule) locks.get(j)))) {
				for (int i = 0; i < graph.length; i++) {
					if ((graph[i][j] > NO_STATE)
							&& (graph[i][lockIndex] == NO_STATE))
	...
	private Object[] getOwnedLocks(Thread current) {
		...
		if (ownedLocks.size() == 0)
			Assert.isLegal(false,
					"A thread with no locks is part of a deadlock."); //$NON-NLS-1$
	...
	private Thread[] getThreadsInDeadlock(Thread cause) {
		...
		/**
		 * if the thread that caused deadlock doesn't own any locks, then it is
		 * not part of the deadlock (it just caused it because of a rule it
		 * tried to acquire)
		 */
		if (ownsLocks(cause))
			deadlockedThreads.add(cause);
		addCycleThreads(deadlockedThreads, cause);
		return (Thread[]) deadlockedThreads
				.toArray(new Thread[deadlockedThreads.size()]);
	}
	...
	private Thread[] getThreadsOwningLock(ISchedulingRule rule) {
		...
		if ((blocking.size() == 0) && (JobManager.DEBUG_LOCKS))
			System.out.println("Lock " //$NON-NLS-1$
					+ rule
					+ " is involved in deadlock but is not owned by any thread."); //$NON-NLS-1$
		if ((blocking.size() > 1) && (rule instanceof ILock)
				&& (JobManager.DEBUG_LOCKS))
			System.out.println("Lock " //$NON-NLS-1$
					+ rule
					+ " is owned by more than 1 thread, but it is not a rule."); //$NON-NLS-1$
	...
	private Object getWaitingLock(Thread current) {
		...
		// it can happen that a thread is not waiting for any lock (it is not
		// really part of the deadlock)
		return null;
	}

	/**
	 * Returns the index of the given lock in the lock array. If the lock is not
	 * present in the array, it is added to the end.
	 */
	...
	public boolean isEmpty() {
		return (locks.size() == 0) && (lockThreads.size() == 0)
				&& (graph.length == 0);
	}
	 ...
	 */
	public void lockAcquired(Thread owner, ISchedulingRule lock) {
		...
		/**
		 * acquire all locks that conflict with the given lock or conflict with
		 * a lock the given lock will acquire implicitly (locks are acquired
		 * implicitly when a conflicting lock is acquired)
		 */
		ArrayList conflicting = new ArrayList(1);
		// only need two passes through all the locks to pick up all conflicting
		// rules
		int NUM_PASSES = 2;
		...
		for (int i = 0; i < NUM_PASSES; i++) {
			for (int k = 0; k < conflicting.size(); k++) {
				...
				for (int j = 0; j < locks.size(); j++) {
					...
					if (current.isConflicting(possible)
							&& !conflicting.contains(possible)) {
	 ...
	 */
	public void lockReleased(Thread owner, ISchedulingRule lock) {
		...
		if (threadIndex < 0) {
			if (JobManager.DEBUG_LOCKS)
				System.out.println("[lockReleased] Lock " //$NON-NLS-1$
						+ lock + " was already released by thread " //$NON-NLS-1$
						+ owner.getName());
			return;
		}
		if (lockIndex < 0) {
			if (JobManager.DEBUG_LOCKS)
				System.out.println("[lockReleased] Thread " //$NON-NLS-1$
						+ owner.getName() + " already released lock " //$NON-NLS-1$
						+ lock);
			return;
		}
		...
		// or release all rules that are owned by the given thread, if we are
		// releasing a rule
		for (int j = 0; j < graph[threadIndex].length; j++) {
			if ((lock.isConflicting((ISchedulingRule) locks.get(j)))
					|| (!(lock instanceof ILock)
							&& !(locks.get(j) instanceof ILock)
							&& (graph[threadIndex][j] > NO_STATE))) {
				if (graph[threadIndex][j] == NO_STATE) {
					if (JobManager.DEBUG_LOCKS)
						System.out.println(
								"[lockReleased] More releases than acquires for thread " //$NON-NLS-1$
										+ owner.getName() + " and lock " //$NON-NLS-1$
										+ lock);
				} else {
					graph[threadIndex][j]--;
				}
		...
		// if this thread just released the given lock, try to simplify the
		// graph
	 ...
	 * acquired.
	 */
	public void lockReleasedCompletely(Thread owner, ISchedulingRule rule) {
		int ruleIndex = indexOf(rule, false);
		int threadIndex = indexOf(owner, false);
		// need to make sure that the given thread and rule were not already
		// removed from the graph
		if (threadIndex < 0) {
			if (JobManager.DEBUG_LOCKS)
				System.out.println("[lockReleasedCompletely] Lock " //$NON-NLS-1$
						+ rule + " was already released by thread " //$NON-NLS-1$
						+ owner.getName());
			return;
		}
		if (ruleIndex < 0) {
			if (JobManager.DEBUG_LOCKS)
				System.out.println("[lockReleasedCompletely] Thread " //$NON-NLS-1$
						+ owner.getName() + " already released lock " //$NON-NLS-1$
						+ rule);
			return;
		}
		/**
		 * set all rules that are owned by the given thread to NO_STATE (not
		 * just rules that conflict with the rule we are releasing) if we are
		 * releasing a lock, then only update the one entry for the lock
		 */
		for (int j = 0; j < graph[threadIndex].length; j++) {
			if (!(locks.get(j) instanceof ILock)
					&& (graph[threadIndex][j] > NO_STATE))
				graph[threadIndex][j] = NO_STATE;
		}
	 ...
	 * Update the graph.
	 */
	public Deadlock lockWaitStart(Thread client, ISchedulingRule lock) {
		...
		if (JobManager.DEBUG_DEADLOCK)
			throw new IllegalStateException(
					"Deadlock detected. Caused by thread " //$NON-NLS-1$
							+ client.getName() + '.');
		// Update the graph to indicate that the locks will now be suspended.
		// To indicate that the lock will be suspended, we set the thread to
		// wait for the lock.
	 ...
	 * graph.
	 */
	public void lockWaitStop(Thread owner, ISchedulingRule lock) {
		int lockIndex = indexOf(lock, false);
		int threadIndex = indexOf(owner, false);
		// make sure the thread and lock exist in the graph
		if (threadIndex < 0) {
			if (JobManager.DEBUG_LOCKS)
				System.out.println("Thread " //$NON-NLS-1$
						+ owner.getName() + " was already removed."); //$NON-NLS-1$
			return;
		}
		if (lockIndex < 0) {
			if (JobManager.DEBUG_LOCKS)
				System.out.println("Lock " //$NON-NLS-1$
						+ lock + " was already removed."); //$NON-NLS-1$
			return;
		}
		if (graph[threadIndex][lockIndex] != WAITING_FOR_LOCK)
			Assert.isTrue(false,
					"Thread " //$NON-NLS-1$
							+ owner.getName() + " was not waiting for lock " //$NON-NLS-1$
							+ lock.toString() + " so it could not time out."); //$NON-NLS-1$
		graph[threadIndex][lockIndex] = NO_STATE;
		reduceGraph(threadIndex, lock);
	}
	...
	/**
	 * Returns true IFF the given thread owns a single real lock. A real lock is
	 * a lock that can be suspended.
	 */
	...
	/**
	 * Returns an array of real locks that are owned by the given thread. Real
	 * locks are locks that implement the ILock interface and can be suspended.
	 */
	private ISchedulingRule[] realLocksForThread(Thread owner) {
		...
		if (ownedLocks.size() == 0)
			Assert.isLegal(false,
					"A thread with no real locks was chosen to resolve deadlock."); //$NON-NLS-1$
		return (ISchedulingRule[]) ownedLocks
				.toArray(new ISchedulingRule[ownedLocks.size()]);
	}
	...
	private void reduceGraph(int row, ISchedulingRule lock) {
		...
		/**
		 * find all columns that could possibly be empty (consist of locks which
		 * conflict with the given lock, or of locks which are rules)
		 */
		...
		/**
		 * Check if the possibly empty columns are actually empty. If a column
		 * is actually empty, remove the corresponding lock from the list of
		 * locks Start at the last column so that when locks are removed from
		 * the list, the index of the remaining locks is unchanged. Store the
		...
		// new graph (the list of locks and the list of threads are already
		// updated)
		...
		// the number of rows we need to skip to get the correct entry from the
		// old graph
		...
		for (int i = 0; i < graph.length - numRowsSkipped; i++) {
			...
			// the number of columns we need to skip to get the correct entry
			// from the old graph
		...
		graph = tempGraph;
		Assert.isTrue(numThreads == graph.length,
				"Rows and threads don't match."); //$NON-NLS-1$
		Assert.isTrue(numLocks == ((graph.length > 0) ? graph[0].length : 0),
				"Columns and locks don't match."); //$NON-NLS-1$
	}
	...
	private void reportDeadlock(Deadlock deadlock) {
		String msg = "Deadlock detected. All locks owned by thread " //$NON-NLS-1$
				+ deadlock.getCandidate().getName() + " will be suspended."; //$NON-NLS-1$
		MultiStatus main = new MultiStatus(JobManager.PI_JOBS,
				JobManager.PLUGIN_ERROR, msg, new IllegalStateException());
		...
		for (int i = 0; i < threads.length; i++) {
			...
			buf.append(waitLock);
			Status child = new Status(IStatus.ERROR, JobManager.PI_JOBS,
					JobManager.PLUGIN_ERROR, buf.toString(), null);
			main.add(child);
		}
	}
	 ...
	 * actual locks and not rules). Return the first thread in the array by
	 * default.
	 */
	private Thread resolutionCandidate(Thread[] candidates) {
		...
		// next look for any candidate with a real lock (a lock that can be
		// suspended)
		for (int i = 0; i < candidates.length; i++) {
	 ...
	 */
	private void setToWait(Thread owner, ISchedulingRule lock,
			boolean suspend) {
		boolean needTransfer = false;
		/**
		 * if we are adding an entry where a thread is waiting on a scheduling
		 * rule, then we need to transfer all positive entries for a conflicting
		 * rule to the newly added rule in order to synchronize the graph.
		 */
	...
	public String toDebugString() {
		...
		for (int i = 0; i < graph.length; i++) {
			out.print(" " //$NON-NLS-1$
					+ ((Thread) lockThreads.get(i)).getName() + " : "); //$NON-NLS-1$
			for (int j = 0; j < graph[i].length; j++) {
				out.print(" " //$NON-NLS-1$
						+ graph[i][j] + ',');
			}
